...
# These values should match the saved data being loaded
ACQTIME = 5
SPS = 860  # Samples per second
nsamples = ACQTIME * SPS
sinterval = 1.0 / SPS
...
print(f"Loading relaxed data from: {relaxed_file}")
file1 = open(relaxed_file, 'rb')
relaxed_data = pickle.load(file1)
file1.close()
relaxed_data = np.array(relaxed_data)

print(f"Loading concentrated data from: {concentrated_file}")
file2 = open(concentrated_file, 'rb')
concentrated_data = pickle.load(file2)
file2.close()
concentrated_data = np.array(concentrated_data)
...
for i, time_series in enumerate(relaxed_data):
    ...
    relaxed_rms[i] = rms
    if i == 0:
        ps1 = ps  # store the first power spectrum values to plot
for i, time_series in enumerate(concentrated_data):
    ps, rms = rms_voltage_power_spectrum(time_series, freq_min, freq_max, SPS, nsamples)
    concentrated_rms[i] = rms
    if i == 0:
...
print("Mean RMS for relaxed state:", np.mean(relaxed_rms))
print("Mean RMS for concentrated state:", np.mean(concentrated_rms))
