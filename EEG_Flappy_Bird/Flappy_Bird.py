...
SCORE_RATE = 0.005  # Rate at which score increases per tick
INITIAL_BIRD_X = 50  # initial y coordinate of the bird
VELOCITY = 0.5  # basically the proportionality factor between the eeg signal and bird velocity in gameplay
GRAVITY = 1.1  # Rate at which the bird falls
...
# create a new pipe with random height in the pipe height list
def create_pipe():
    random_height = random.choice(PIPE_HEIGHT)
    bottom_pipe = pipe_surface.get_rect(midtop=(350, random_height))
    top_pipe = pipe_surface.get_rect(midbottom=(350, random_height - PIPE_SEP))
...
# take the pipe list as an argument, move them to left (the same rate as floor?)
def move_pipes(pipes):
    for pipe in pipes:
        pipe.centerx -= PIPE_SPEED
    return pipes

# draw pipes in the pipe list
def draw_pipes(pipes):
    for pipe in pipes:
        # draw bottom pipe
        if pipe.bottom >= 512:
...
while True:
    ...
    for event in pygame.event.get():  # all events that pygame can detect
        ...
        if event.type == SPAWNPIPE:
            pipe_list.extend(create_pipe())  # extend allows the list to include whatever that is in the tuple
