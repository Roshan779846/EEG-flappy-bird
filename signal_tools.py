...
def gaussian_eval(relaxed, concentrated):
    ...
    # Select the real roots in the middle of the two mean values.
    intersection = [result.real for result in results if result.imag == 0 and r_mean < result.real < c_mean]
...
def brain_signal_extraction(time_series, min_freq, max_freq, freq):
    ...
    time_series_fft = fft(time_series)
    freq_mask = (freq >= min_freq) & (freq <= max_freq)
    time_series_fft[(~freq_mask)] = 0
    brain_wave = ifft(time_series_fft).real
    return brain_wave

# Set the board_id based on the operating system
if platform.system() == 'Windows':
    board_id = "Windows-PC"
elif platform.system() == 'Linux':
    board_id = "Linux-PC"
else:
    board_id = "Unknown-Platform"

print(f"Detected board ID: {board_id}")

# Sample usage of the functions
time_series = np.random.rand(1000)
ps, rms = rms_voltage_power_spectrum(time_series, 5, 15, 1000, len(time_series))
print(f"Power Spectrum: {ps}")
print(f"RMS Voltage: {rms}")

intersection = gaussian_eval(np.random.rand(500), np.random.rand(500))
print(f"Gaussian Evaluation: {intersection}")

freq = fftfreq(len(time_series), d=1.0/1000)
brain_wave = brain_signal_extraction(time_series, 8, 12, freq)
print(f"Brain Wave Signal: {brain_wave}")
