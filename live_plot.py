...
import os
import sys
sys.path.insert(1, os.path.dirname(os.getcwd()))  # Allows importing files from the parent folder
import time
import numpy as np
import matplotlib.pyplot as plt
from matplotlib.animation import FuncAnimation
...
# Manually set the board ID based on the platform
...
def update(frame):
    ...
    chan_value = np.random.uniform(-1, 1)  # Random value for testing
    t.append(st - ot)
    y.append(chan_value * (4.096 / 2) - 3.3)  # Adjusted for testing
