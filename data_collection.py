...
# Replace this with the correct serial port and baud rate for your EEG device
...
stopper = 'n'
while stopper != 'y':  # Loops every time the user records data
    input('Press <Enter> to start %.1f s data acquisition...' % ACQTIME)
    print()
    ...
    for i in range(nsamples):  # Collects data every sinterval
        ...
        print("Raw data:", data)  # Print raw data

        try:
            data_point = json.loads(data)
            time_series[i] = data_point['ch6']  # Replace with the correct key for your EEG channel
            while (time.perf_counter() - start_time) <= sinterval:
                pass
